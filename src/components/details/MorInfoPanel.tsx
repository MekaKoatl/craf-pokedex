...
function LocationChips({ locations }: { locations: string[] }) {
  ...
  const SHOW = 8;
  const visible = showAll ? locations : locations.slice(0, SHOW);
  return (
    <div className="flex flex-wrap gap-1">
      {visible.map((n) => (
        <span
          key={n}
          className="inline-block bg-gray-100 border border-gray-200 rounded px-2 py-0.5 text-xs capitalize"
        >
          {n}
        </span>
      ))}
      {locations.length > SHOW && (
        <span
          onClick={() => setShowAll((v) => !v)}
          className="text-red-400 text-xs cursor-pointer hover:underline"
        >
          {showAll ? "show less" : `+${locations.length - SHOW} more`}
        </span>
      )}
    </div>
  );
...
export function MoreInfoPanel({
  ...
  const { data: locations } = useQuery({
    ...
    queryFn: async () => {
      ...
      return [
        ...new Set(
          data.map((l: { location_area: { name: string } }) =>
    ...
    <div className="mb-8">
      ...
      {open && (
        <div className="mt-3 grid grid-cols-2 sm:grid-cols-3 gap-4 bg:red">
          ...
          <Card label="Base Experience">
            {pokemon.base_experience
              ? `${pokemon.base_experience} XP`
              : "Unknown"}
          </Card>
          <Card label="Capture Rate">
            {species.capture_rate} ({catchPct}%)
          </Card>
          <Card label="Shape">
          ...
          <Card label="Pokédex Color">
            <span
              className="inline-block w-3.5 h-3.5 rounded-sm mr-1.5 align-middle border border-gray-200"
              style={{ background: COLOR_MAP[colorName] ?? "#ccc" }}
            />
            <span className="capitalize">{colorName}</span>
          </Card>
          <Card label="Base Friendship">
            {species.base_happiness ?? "Unknown"}
          </Card>
          <Card label="Leveling Rate">
            <span className="capitalize">
